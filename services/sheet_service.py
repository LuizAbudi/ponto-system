...
class SheetService:
    ...
    def adjust_times(self, rows):
        ...
        for i in range(1, len(rows)):
            previous_end_time = datetime.strptime(rows[i-1].hora_fim_formatada, '%d/%m/%Y %H:%M:%S')
            current_start_time = datetime.strptime(rows[i].hora_inicio_formatada, '%d/%m/%Y %H:%M:%S')

            if current_start_time <= previous_end_time:
                new_start_time = previous_end_time + timedelta(seconds=1)
                rows[i].hora_inicio_formatada = new_start_time.strftime('%d/%m/%Y %H:%M:%S')
                rows[i].hora_inicio = new_start_time.strftime('%H:%M')
                total_seconds_added += 1

        if rows:
            last_task_end_time = datetime.strptime(rows[-1].hora_fim_formatada, '%d/%m/%Y %H:%M:%S')
            new_end_time = last_task_end_time + timedelta(seconds=total_seconds_added)
            rows[-1].hora_fim_formatada = new_end_time.strftime('%d/%m/%Y %H:%M:%S')
